...
mybeta1 <- cor(yc,xc) * sd(yc)/sd(xc)
c(beta1, coef(lm(y ~ x))[2],mybeta1,coef(lm(yc ~ xc))[2])
...
g = g + ylab("Price (SIN $)")
g = g + geom_point(size = 7, colour = "brown", alpha=0.5)
g = g + geom_point(size = 5, colour = "orange", alpha=0.2)
...
coef(fit)[1] + coef(fit)[2] * newx # Using formulae
predict(fit, newdata = data.frame(carat = newx)) # Using fit - predict()
...
# Diamond data residual plot
mean(diamond$price)
# lm with constant gives just the intercept which is the mean of X
